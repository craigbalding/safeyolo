 ...
 *
 * Prerequisites:
 *   - SafeYolo running with: safeyolo watch --socket /tmp/safeyolo_test.sock
 */

const SOCKET_PATH = process.env.SAFEYOLO_SOCKET || "/tmp/safeyolo_test.sock";
  ...
  const socket = await Bun.connect({
    ...
    socket: {
      ...
      close(socket) {
        ...
        console.log(`\n[UDS Consumer] Received ${eventCount} events`);
      },
    },
  });
