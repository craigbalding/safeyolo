...
3. In different `modules of this scope`, typically the submodule/package folders, there is a subfolder (likely under an intermediate `_@dev` subfolder) whose name starts `archive.`.
    ...
    * **⚠️ NEVER use in reasoning or as a context files present in paths where the name of one of the folders starts with `archive.`**, like:
        - `_@dev/archive.agentic-plans-sessions` or
        - `_@dev/archive.user-plans-specs` (where square brackets are relevant part of the pathname),
