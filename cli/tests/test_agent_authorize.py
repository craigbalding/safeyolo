...
from safeyolo.agent_configuration import _parse_mount, _validate_instance_name
...
def _write_conditional_operator_contract_service(config_dir: Path) -> None:
    _write_service(
        config_dir,
        "heartbeat",
        {
...
class TestContractBindingGuidance:
    ...
    @pytest.mark.parametrize(
        ("mode", "expect_incomplete"),
        [("basic", False), ("advanced", True)],
    )
    def test_conditional_operator_binding_uses_active_values(
        self, cli_runner, tmp_config_dir, mode, expect_incomplete
    ):
        existing_binding = {
            "binding_id": "cb-existing",
            "service": "heartbeat",
        ...
        assert result.exit_code == 0
        assert ("Setup incomplete" in result.output) is expect_incomplete
        assert ("advanced_target_id" in result.output) is expect_incomplete
...
class TestParseMountSecurity:
    def test_protected_path_requires_read_only(self, tmp_path, monkeypatch):
        ...
        with patch("safeyolo.agent_configuration.is_path_protected", return_value=str(tmp_path), autospec=True,):
            with pytest.raises(typer.Exit):
    ...
    def test_normal_path_writable(self, tmp_path):
        ...
        host_dir.mkdir()
        from unittest.mock import patch
        with patch("safeyolo.agent_configuration.is_path_protected", return_value=None, autospec=True,):
