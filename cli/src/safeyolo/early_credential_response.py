"""Respond to a denied streamed request before mitmproxy opens the origin.
...
keeps the stock path for every flow without a credential-guard head denial.
...
def install_early_credential_response() -> None:
    """Install the per-flow early denial path before the proxy starts."""
    if version("mitmproxy") != "12.2.3":
        raise RuntimeError("early credential response requires reviewed mitmproxy 12.2.3")

    original = HttpStream.start_request_stream

    def start_request_stream(self):
        if not self.flow.metadata.get("credential_guard_head_denied"):
        ...
        if self.flow.response is None:
            raise RuntimeError("credential guard marked a head denial without a response")
