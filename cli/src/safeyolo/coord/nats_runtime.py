...
# Internal test contract. Real-NATS tests set a unique instance token and a
# non-home coord data directory. NATS then asks the kernel for both ports and
# publishes the selected endpoints through its ports-file mechanism. This is
# deliberately not a production configuration option: without the marker,
# the public runtime remains on the fixed defaults above.
_TEST_NATS_INSTANCE_ENV = "SAFEYOLO_NATS_TEST_INSTANCE"
_TEST_DYNAMIC_PORT = -1
...
def _test_instance() -> str | None:
    """Return the validated real-NATS test owner token, if configured.

    Test mode fails closed before any NATS path helper can fall back to the
    operator's home directory. A test must name an absolute, non-home coord
    directory and a compact token that can safely form part of server_name.
    """
    instance = os.environ.get(_TEST_NATS_INSTANCE_ENV)
    if instance is None:
        return None
    if re.fullmatch(r"[A-Za-z0-9_-]{1,32}", instance) is None:
        raise RuntimeError(
            f"{_TEST_NATS_INSTANCE_ENV} must contain 1-32 letters, digits, "
    ...
    if not override:
        raise RuntimeError(
            f"{_TEST_NATS_INSTANCE_ENV} requires SAFEYOLO_COORD_DATA_DIR; "
            "refusing to use the operator's default coord state"
        )
    data_dir = Path(override)
    if not data_dir.is_absolute():
        raise RuntimeError(
            "real-NATS tests require an absolute SAFEYOLO_COORD_DATA_DIR"
        )

    resolved = data_dir.resolve(strict=False)
    operator_root = (Path.home() / ".safeyolo").resolve(strict=False)
    if resolved == operator_root or operator_root in resolved.parents:
        raise RuntimeError(
            "real-NATS tests cannot use state under the operator's ~/.safeyolo"
        )
    return instance
...
def nats_ports_dir() -> Path:
    """Directory for NATS-owned dynamic endpoint files in test mode."""
    return nats_root() / "ports"


def nats_test_endpoints_path() -> Path:
    """Last dynamic endpoints, retained for unavailable-path tests."""
    return nats_root() / "test-endpoints.json"
...
def _new_server_name() -> str:
    ...
    if instance is not None:
        return f"safeyolo-test-{instance}-{secrets.token_hex(8)}"
...
def write_config(server_name: str) -> Path:
    ...
    """
    ensure_credentials()
    client_port, monitor_port = _configured_ports()
    config = f"""# Generated by SafeYolo coord/nats_runtime.py. Do not edit by hand.
listen: {NATS_LISTEN_HOST}:{client_port}
http: {NATS_LISTEN_HOST}:{monitor_port}
...
def _write_pidfile(
    ...
    *,
    ...
    if client_port is not None and monitor_port is not None:
        payload["client_port"] = client_port
        payload["monitor_port"] = monitor_port
    if test_instance is not None:
        payload["test_instance"] = test_instance
    path = nats_pid_path()
...
def _validated_test_ports(
    client_port: object, monitor_port: object
) -> tuple[int, int] | None:
    if (
        not isinstance(client_port, int)
        or isinstance(client_port, bool)
        or not isinstance(monitor_port, int)
        or isinstance(monitor_port, bool)
        or not 1 <= client_port <= 65535
        or not 1 <= monitor_port <= 65535
        or client_port == monitor_port
        or client_port == NATS_CLIENT_PORT
        or monitor_port == NATS_MONITOR_PORT
    ):
        return None
    return (client_port, monitor_port)


def _test_ports_from_pidfile(stored: dict | None) -> tuple[int, int] | None:
    """Return endpoints only for the current test-owned server record."""
    instance = _test_instance()
    if instance is None or stored is None:
        return None
    if stored.get("test_instance") != instance:
        return None
    if not stored["server_name"].startswith(f"safeyolo-test-{instance}-"):
        return None
    return _validated_test_ports(
        stored.get("client_port"), stored.get("monitor_port")
    )


def _write_test_endpoints(ports: tuple[int, int]) -> None:
    """Persist the last test endpoint so down-server paths stay realistic."""
    instance = _test_instance()
    if instance is None:
        return
    _secure_mkdir(nats_root())
    ...
    descriptor, staged_name = tempfile.mkstemp(
        dir=nats_root(), prefix=".test-endpoints.", suffix=".tmp"
    )
    try:
        os.fchmod(descriptor, _SECURE_FILE_MODE)
        with os.fdopen(descriptor, "w") as staged:
            staged.write(json.dumps(payload) + "\n")
            staged.flush()
            os.fsync(staged.fileno())
        os.replace(staged_name, nats_test_endpoints_path())
    finally:
        Path(staged_name).unlink(missing_ok=True)


def _read_test_endpoints() -> tuple[int, int] | None:
    instance = _test_instance()
    if instance is None:
        return None
    try:
        data = json.loads(nats_test_endpoints_path().read_text())
    except (OSError, json.JSONDecodeError):
        return None
    if not isinstance(data, dict) or data.get("test_instance") != instance:
        return None
    return _validated_test_ports(
        data.get("client_port"), data.get("monitor_port")
    )


def _parse_dynamic_port(url: object, scheme: str) -> int | None:
    if not isinstance(url, str):
        return None
    parsed = urlsplit(url)
    if parsed.scheme != scheme or parsed.hostname != NATS_LISTEN_HOST:
        return None
    try:
        port = parsed.port
    except ValueError:
...
def _read_dynamic_ports(pid: int) -> tuple[int, int] | None:
    """Read the kernel-selected endpoints from NATS's test-owned file."""
    if _test_instance() is None:
        return None
    ports_dir = nats_ports_dir()
    try:
        matches = [
            path
            for path in ports_dir.glob(f"*_{pid}.ports")
    ...
    if not isinstance(data, dict):
        return None
    nats_urls = data.get("nats")
    monitor_urls = data.get("monitoring")
    if (
        not isinstance(nats_urls, list)
        or len(nats_urls) != 1
        or not isinstance(monitor_urls, list)
        or len(monitor_urls) != 1
    ):
        return None
    client_port = _parse_dynamic_port(nats_urls[0], "nats")
    monitor_port = _parse_dynamic_port(monitor_urls[0], "http")
    if (
        client_port is None
        or monitor_port is None
        or client_port == monitor_port
        or client_port == NATS_CLIENT_PORT
        or monitor_port == NATS_MONITOR_PORT
    ):
...
def _remove_dynamic_ports_file(pid: int) -> None:
    """Remove only this test-owned NATS process's endpoint file."""
    if _test_instance() is None:
        return
    with contextlib.suppress(OSError):
        for path in nats_ports_dir().glob(f"*_{pid}.ports"):
            if path.is_file() and not path.is_symlink():
                path.unlink(missing_ok=True)


def _varz(
    timeout: float = 1.0, *, monitor_port: int | None = None
) -> dict | None:
    ...
    if monitor_port is None:
        if _test_instance() is not None:
            ports = _test_ports_from_pidfile(_read_pidfile())
            if ports is None:
                return None
            monitor_port = ports[1]
        else:
    ...
    try:
        conn = http.client.HTTPConnection(
            NATS_LISTEN_HOST, monitor_port, timeout=timeout
        )
...
def _verified_ownership() -> dict | None:
    ...
    if not _pid_alive(stored["pid"]):
        return None
    if _test_instance() is not None:
        ports = _test_ports_from_pidfile(stored)
        if ports is None:
            return None
        live = _varz(monitor_port=ports[1])
    else:
        live = _varz()
    if (
        live is None
...
def _start_server_locked(ready_timeout: float) -> int:
    ...
    if stale is not None:
        _remove_dynamic_ports_file(stale["pid"])
    ...
    if test_instance is not None:
        _secure_mkdir(nats_ports_dir())
        command.extend(["--ports_file_dir", str(nats_ports_dir())])
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            if test_instance is not None:
                if ports is None:
                    ports = _read_dynamic_ports(proc.pid)
                if ports is None:
                    time.sleep(0.05)
                    continue
            else:
                ports = (NATS_CLIENT_PORT, NATS_MONITOR_PORT)
        ...
        _reap_child(proc)
        _remove_dynamic_ports_file(proc.pid)
        nats_pid_path().unlink(missing_ok=True)
        raise
...
def _stop_server_locked() -> bool:
    ...
    if verified is None:
        ...
        if stored is not None and _pid_alive(stored["pid"]):
            raise WedgedNatsServer(
                ...
                f"({NATS_LISTEN_HOST}:"
                f"{stored.get('monitor_port', NATS_MONITOR_PORT)})."
            )
        if stored is not None:
            _remove_dynamic_ports_file(stored["pid"])
        nats_pid_path().unlink(missing_ok=True)
        return False
    pid = verified["pid"]
    try:
    ...
    _remove_dynamic_ports_file(pid)
    nats_pid_path().unlink(missing_ok=True)
    return True
...
def status() -> dict:
    ...
    if _test_instance() is not None:
        ports = _test_ports_from_pidfile(stored)
        listen = (
            f"{NATS_LISTEN_HOST}:{ports[0]}"
            if ports is not None
            else f"{NATS_LISTEN_HOST}:auto"
        )
    else:
...
def client_url() -> str:
    ...
    if _test_instance() is not None:
        ports = _test_ports_from_pidfile(_read_pidfile())
        if ports is None:
            ports = _read_test_endpoints()
        if ports is None:
            raise RuntimeError("test-owned NATS client endpoint is unavailable")
        client_port = ports[0]
    else:
        client_port = NATS_CLIENT_PORT
    return f"nats://{NATS_LISTEN_HOST}:{client_port}"
