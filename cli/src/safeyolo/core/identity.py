...
def snapshot_replay_attribution(
    flow: http.HTTPFlow,
    discovery: AgentLookup | None = None,
) -> TrafficAttribution:
    """Capture the trusted replay initiator without changing evidence ownership.

    Operator replay starts another request on an existing or copied flow.
    Retain its original trusted identity and any late-change quarantine, but
    replace the attribution snapshot after the operator addon marks the replay.
    """
    attribution = attribute_traffic(flow, flow_identity(flow, discovery))
    flow.metadata[ATTRIBUTION_SNAPSHOT_KEY] = _attribution_snapshot(attribution)
    return attribution
