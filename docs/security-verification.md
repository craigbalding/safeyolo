...
| Linux namespace lifetime | The per-agent holder stays alive while the agent runs and is killed on stop; status and exec use it to enter the user and network namespaces | [cli/src/safeyolo/platform/linux.py](../cli/src/safeyolo/platform/linux.py) |
