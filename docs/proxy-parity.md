...
| D2 | Production sets `stream_large_bodies=10m`; the SSE addon sets response streaming. Its docstring says request bodies remain fully inspected, but large-body transport streaming and scanner `get_text()` require separate examination. Buffered-body hooks cannot establish inspection of bytes already forwarded. | Live tests show that a 10 MiB request body is scanned and blocked before origin contact; an over-threshold body (known-length or chunked) forwards byte-exact with an empty retained snippet and no scan claim; URL scope still blocks before upload; and an enabled SSE response streams with an unavailable body (188a5291). The Python credential guard decides detected HTTP/1 header credentials at the head for announced large and unannounced chunked bodies. A denied head gets a local response before origin contact through a per-flow integration with mitmproxy 12.2.3's private `HttpStream.start_request_stream`; revalidate the integration before upgrading the dependency. Allowed bodies retain the existing streaming path. In a separate framing control, an origin waiting for the terminating chunk timed out on allowed small and 12 MiB requests before and after the credential-guard change; the same origin returned 200 for a direct chunked request. HTTP/2, allowed chunked completion, and body inspection beyond the configured window remain coverage gaps; do not claim full inspection from hook execution. |
