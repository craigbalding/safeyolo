...
| 16. `addons/credential_guard.py` | Detect HTTP request-header credentials, evaluate destination-first permissions and budgets, and return configured block/warn outcomes. | Native H1 request-head and H2-inside-owned-TLS request-head integration run after network/circuit admission and before observation/body/dial; policy events use keyed fingerprints and identity conflicts fail closed. Bodies and WebSockets belong to the pattern scanner. | Native header detection/selection and policy evaluation are active for `policy_file`; malformed header bytes cross a reversible private source-text adapter and controlled H1/H2 wires prove matching warn/block plus nonmatching raw forwarding. Body/query scanning remains outside this adapter. Python D33 Unicode categories, backreferences, nesting, large-message limits, HTTP/WS fragmentation, compression and cancellation parity remain open. | `test_credential_guard.py`, `test_credential_catalog.py`, `test_policy_budget_contract.py`, `credential_http.rs`, `http::traffic::tests::ordinary_h2_inside_owned_tls_logs_inner_exchange_only` |
...
Owner candidate checks cover 26 scanner tests, including seven actual Python
oracles, 315 inherited engine tests, allocation/cancellation regressions and a
...
Named Unicode escapes, other Unicode/class behavior, nesting, delegated-search
cancellation and the HTTP text adapter remain unresolved. Full details and
pinned data provenance are in the [engine patch](../proxy/vendor/fancy-regex/SAFEYOLO.md).
...
Unicode categories/backreferences/nesting, HTTP/WS decoding, fragmentation,
compression and cancellation remain explicit compatibility gaps. Accepted
