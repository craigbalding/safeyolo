...
| D2 | Production sets `stream_large_bodies=10m`; the SSE addon sets response streaming. Its docstring says request bodies remain fully inspected, but large-body transport streaming and scanner `get_text()` require separate examination. Buffered-body hooks cannot establish inspection of bytes already forwarded. | For an announced HTTP/1 body above 10 MiB, the Python credential guard now decides detected request-header credentials before streaming. A denied head gets a local response before origin contact through a per-flow integration with mitmproxy 12.2.3's private `HttpStream.start_request_stream`; revalidate that integration before upgrading the locked dependency. Allowed large bodies still stream. HTTP/2, unannounced/chunked bodies, and body inspection beyond the configured window remain separate coverage gaps; do not claim full inspection from hook execution. |
