...
| D2 | Production sets `stream_large_bodies=10m`; the SSE addon sets response streaming. Its docstring says request bodies remain fully inspected, but large-body transport streaming and scanner `get_text()` require separate examination. Buffered-body hooks cannot establish inspection of bytes already forwarded. | For announced large HTTP/1 bodies and unannounced HTTP/1 chunked bodies, the Python credential guard decides detected request-header credentials at the head. A denied head gets a local response before origin contact through a per-flow integration with mitmproxy 12.2.3's private `HttpStream.start_request_stream`; revalidate that integration before upgrading the locked dependency. Allowed large and chunked bodies keep their existing transport streaming behavior. HTTP/2 and body inspection beyond the configured window remain separate coverage gaps; do not claim full inspection from hook execution. |
