...
| POST `/admin/agents/{agent}/desktop/present` and retained agent collaboration routes | Partly implemented | Native route accepts only the optional `approval_request_id` correlation field; caller-supplied target/command arguments are rejected before the host helper. The retained host boundary and #629 running workflow witness prove pending approval, exact request-ID handoff, listener-derived target binding, fixture helper invocation and truthful unavailable/failure audit. Real supported-platform presentation and the remaining collaboration/persistence cases remain open. |
