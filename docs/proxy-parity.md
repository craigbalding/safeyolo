...
| D8 | The default production command only explicitly selects lazy connections when sinkhole routing is enabled. Earlier live denial fixtures varied eager/lazy for CONNECT, not every plain-HTTP security decision. | The shared Linux outbound-effects fixture now uses `strace` with a live permitted-parent control. Both backends show only the two control IP `connect` attempts across reserved API/probe and CONNECT requests, a malformed local request, and denied plain HTTP and CONNECT. The parent accepts and application requests also contain only the controls. Python uses eager connections in this case. DNS absence remains unverified because the fixture has no independent live resolver control; neither zero parent accepts nor `proxy.egress` proves it. Other plain-HTTP decisions and platforms remain open. |
