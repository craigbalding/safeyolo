...
| D74 | Both proxies folded repeated Authorization fields before checking for a reserved gateway token. When an ordinary value preceded a token, the gateway missed it and allowed the request to reach an owned origin with the token in its Authorization fields. A single field with a comma-separated ordinary value before the token had the same effect. Both proxies also forwarded a token after a tab instead of a space. | The gateway fallback now examines original header fields, comma-separated values and whitespace between a scheme and token. It denies unresolved reserved tokens before parent contact. The [raw gateway fixture](../tests/proxy_migration/test_raw_http_gateway_boundary.py) checks both field orders, the combined and tab-separated values, a plain duplicate-value control, and zero upstream accepts on denial. A valid gateway request still replaces its token with the vaulted credential; an ordinary signed target keeps its raw path, query order and body. Independent acceptance remains pending. |
