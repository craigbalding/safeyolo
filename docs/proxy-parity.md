...
| D70 | For an absolute-form request to an allowed host with a forbidden Host field, the Python proxy forwards the allowed credential and forbidden Host to a controlled parent. That parent routes by Host, and the forbidden physical origin receives the credential. Inside an allowed CONNECT, the Python proxy also delivers a credential with a changed HTTP/1 Host or HTTP/2 `:authority` to the allowed physical TLS origin under the forbidden virtual host. | Native forwards the absolute-form request with the admitted Host, and it rejects changed inner authorities before application bytes. After validating inner host, port and scheme against CONNECT, native policy now reuses the admitted policy hostname for ASCII case-equivalent Host spellings; other source spellings keep their own inspection input. Both proxies deliver the valid case control and reject a separate conflicting HTTP/2 Host field. The [paired real-proxy fixture](../tests/proxy_migration/test_authority_consistency.py) keeps both Python leaks as strict expected failures. It separately observes the parent CONNECT target, origin TLS Server Name Indication (SNI), a wrong-name certificate rejection with zero application bytes, and exact signed-style path, repeated query and body delivery. A direct trust control proves that the wrong-name certificate is valid for its own name. The parent can deliberately route an admitted CONNECT to another physical socket; SafeYolo verifies that socket's certificate against the admitted name before sending HTTP. Other route and authority combinations remain open under D4. |
