...
| 8. `addons/request_id.py` | Generate request IDs; consume trace opt-in; strip spoofed internal/hop headers; return correlation IDs; relate CONNECT admission and inner requests. | Client-supplied correlation values cannot impersonate trusted observations. | Typed connection/request correlation and header handling; remove addon. | `test_request_id.py`, `test_connect_policy.py`, `test_trace_wire_vocabulary.py`, `tests/proxy_migration/test_http_contract.py` |
...
| 11. `addons/sse_streaming.py` | Stream `text/event-stream` and `application/x-ndjson`; stream JSON when explicitly configured; honor global/domain disablement. | Response-body streaming has an inspection/capture limitation. See discrepancy D2. | Incremental HTTP bodies with explicit coverage; remove callback. | `test_sse_streaming.py` checks hooks; `tests/proxy_migration/test_http_contract.py` checks held SSE event delivery, a stalled reader, an independent request and authenticated admin access on both real backends. These finite cases do not establish all streaming modes or sustained backpressure. |
...
| D2 | Production sets `stream_large_bodies=10m`; the SSE addon sets response streaming. Its docstring says request bodies remain fully inspected, but large-body transport streaming and scanner `get_text()` require separate examination. Buffered-body hooks cannot establish inspection of bytes already forwarded. | Coverage boundary characterized by live request/response tests. Preserve configured streaming behavior and report actual coverage. A concrete bypass of an applicable blocking rule requires a regression and repair; do not claim full inspection from hook execution. Live request/response tests record that a 10 MiB request body is scanned and blocked before the origin dial; an over-threshold body (known-length and chunked) reaches the origin byte-exact with an empty retained snippet and no scan claim; Rust URL scope blocks an over-threshold request before upload; and an enabled SSE response streams with an unavailable body (188a5291). The Python credential guard decides detected HTTP/1 header credentials at the head for announced large and unannounced chunked bodies. A denied head gets a local response before origin contact through per-flow integration with mitmproxy 12.2.3's private `HttpStream.start_request_stream`; revalidate this integration before upgrading the dependency. Allowed bodies retain the existing streaming path. Byte delivery alone did not establish complete HTTP/1.1 framing. The shared [allowed chunked fixture](../tests/proxy_migration/test_allowed_chunked_framing.py) separately requires a buffered request and a request one byte above the threshold to finish at an origin that waits for complete framing. It does not claim body inspection or capture beyond the streaming window. The shared real-proxy HTTP/2 fixture now checks credential-header denial before END_STREAM: a 12 MiB announced body sends only a 21-byte prefix, receives a local denial before an origin application request, and leaves a sibling stream usable. The [shared request-streaming fixture](../tests/proxy_migration/test_request_streaming_inspection.py) runs both proxies against an independent origin and records partial delivery, scanner decisions, and retained evidence. On an announced 10 MiB + 1 byte upload with only a prefix sent, a forbidden credential returns 428, and URL- and header-scope blocks return 403, all with zero origin accepts. Python now checks URL, header, and network policy at requestheaders when the body may stream. Before repair, a held Python URL block opened the origin, and a network-denied upload sent 65,536 body bytes there. A Python chunked URL block also returns at the head; Rust buffers unknown-length uploads until its threshold. Network warn and URL log modes still deliver exact streamed bodies and emit one decision each. Body canaries at 10 MiB - 1 byte and 10 MiB are blocked before origin contact; their retained flows report the full original size, a 16-byte stored prefix, and truncation. At 10 MiB + 1 byte, known-length and chunked bodies reach the origin byte-exact, with partial bytes visible before upload completion. Python records a request-hook `no_match` trace with no scanner audit; Rust records scanner `allow/no_match` against URL and headers with no body. Retained body size, stored, and truncated are all zero because the streamed body is unavailable. With a 16-byte capture limit, permitted 15-, 16-, and 17-byte bodies all produce a body-scope log decision and reach the origin exactly; retained evidence holds the first min(size, 16) bytes and marks only the 17-byte body truncated. No body inspection beyond the buffering threshold is established. |
...
| D4 | CONNECT authority, inner Host/HTTP/2 authority, SNI and actual outbound target are represented separately by the framework. Existing admission tests do not establish the full mismatch matrix. | The shared real-proxy fixture now covers a changed inner port and one trailing DNS root dot in HTTP/1 Host and HTTP/2 `:authority`: both backends reject them before origin HTTP, including a forbidden dotted host. An admitted absolute-form target retains its parent route when Host names the forbidden dotted origin; a direct parent control proves that Host would otherwise select that origin. An explicitly permitted dotted target keeps its signed-style target, body and credential on the allowed route. Python's HTTP parser and Rust's authority validator return a local 400 for absolute-form userinfo with no parent contact; a forbidden dotted target is denied locally. The earlier Python D70 leaks were strict expected failures at PR #755 and are repaired in the current source. Other authority spellings and routes remain open. A changed inner authority must not inherit permission for another destination. Never fall back to opaque transport after parser/TLS failure. |
...
| D8 | The default production command only explicitly selects lazy connections when sinkhole routing is enabled. Earlier live denial fixtures varied eager/lazy for CONNECT, not every plain-HTTP security decision. | The shared Linux outbound-effects fixture uses `strace` with live permitted-parent controls. Both backends show only the two control IP `connect` attempts across reserved API/probe and CONNECT requests, a malformed local request, and denied plain HTTP and CONNECT. The parent accepts and application requests also contain only the controls. Python uses eager connections. A separate direct-route DNS case starts temporary BIND authoritative zones and requires a live loopback `getaddrinfo` control plus permitted requests through each proxy. It checks DNS queries and network syscalls independently while denied and reserved names are used. Hosts without BIND, a loopback resolver route, or port 53 bind skip that case; a skip cannot establish no-DNS behavior. The DNS case covers those named decisions and routes; other plain-HTTP decisions and platforms remain open. |
...
| D10 | Hyper normalizes identical duplicate Content-Length fields and removes Content-Length when Transfer-Encoding controls framing. The old parser rejects those requests. Hyper rejects unequal duplicate lengths. The initial Rust slice also accepted duplicate Host fields. | Protocol difference requiring explicit wire tests. Rust rejects duplicate Host fields before policy or upstream contact. The [raw gateway fixture](../tests/proxy_migration/test_raw_http_gateway_boundary.py) sends Content-Length: 0 with a chunked body through both real proxies. Python rejects it locally; Rust forwards one chunked body with no Content-Length, and the parent and origin record the exact decoded bytes. When a forbidden request follows in the same raw write, Python closes after 400 with no parent contact; Rust closes after its first allowed response. Neither contacts the forbidden origin. A fresh signed request still succeeds. This one framing shape does not establish general request-smuggling resistance. Do not equate normalization to a demonstrated smuggling flaw or add a second parser solely to reproduce every rejection. Verify one unambiguous outbound framing and exact delivered bytes. |
...
| D14 | The initial native policy parser accepted scalar `required`/`bypass` values in list-only fields, potentially allowing traffic with the network guard disabled where Python rejected the document. | Native schema repair requires arrays in these list-only fields and rejects malformed IAM tiers. Conditions that accept either a scalar or a list keep that syntax. [Independent acceptance at `02a054ecd1ea035c1857ef5a1703165c52b9fa66`](https://github.com/craigbalding/safeyolo/issues/621#issuecomment-5849256065) confirmed the list-only result and last-good reload. The `agent-host-bypass-scalar` case is native-stricter: Rust rejects the document before readiness, while Python accepts and ignores the unsupported field but still denies the request (403, zero origin accepts). |
...
| D25 | Before the Python authority repair, the production HTTP/2 tunnel used the admitted CONNECT destination for policy while forwarding a changed inner `:authority`. A forbidden hostname or another port could inherit the tunnel's permission. | Both backends now reject a changed inner `:authority` with 400 before origin application delivery. The [paired HTTP/2 test](../tests/proxy_migration/test_http2_contract.py) checks host and port changes against two controlled origins. Python's HTTP/2 parser still sends a protocol error for a separate Host field disagreement; native returns 400. D70 covers the credential-bearing virtual-host case and the absolute-form parent route. |
| D26 | With Python’s eager connection strategy and an HTTP/1-only origin, client and origin legs negotiate HTTP/1. Rust negotiates HTTP/2 with a capable client and translates that request to an HTTP/1 origin. With Python’s lazy strategy, that same origin also produces an HTTP/2 client leg and an HTTP/1 origin leg. | The shared real-proxy test now asserts observed Application-Layer Protocol Negotiation (ALPN) on both legs for HTTP/2-only and HTTP/1-only origins, with both Python strategies where applicable. No-ALPN traffic and cleartext Unix domain sockets (UDS) still use HTTP/1. Upstream protocol selection follows verified TLS negotiation, without fallback after a TLS or parser error. |
...
| D29 | The dependency's ignore matcher considers the target, connected address, inner Host and TLS SNI. The native path matches configured logical target entries and direct destination IPv4 ranges. | The existing CLI and native configuration boundary normalizes supported exact host forms (trimmed ASCII/IDNA2003 lowercase, explicit ports and deduplication). Focused witnesses cover exact-host/port, host-only address, builtin and constrained CIDR selection, original TLS certificates, canonical direct lifecycle events and removal at reload. A direct connection matched only by its resolved IPv4 peer receives the lifecycle owner after TCP succeeds, while the event retains the logical authority. A logical target or captured SNI entry selects opaque transport through a configured plain or verified TLS parent without bypassing that parent. The parent address and CIDR never match an unrelated logical destination. Inner-Host alias semantics and the remaining D29 matrix remain unresolved; this narrower matcher is not full passthrough acceptance. |
...
| D46 | Before the source repair, numeric loopback aliases, one DNS root dot, mapped IPv6 and DNS aliases could reach a protected loopback port; malformed digit-only extra ports disabled both hooks; and an ephemeral bind or changed port option could leave the live listener unprotected. The source shield now recognizes those numeric and root-dot spellings, retains the configured port when an extra entry cannot be parsed, and checks the actual bound listener port. A DNS alias that resolves to a protected address remains a source gap because the source hook checks the requested hostname before DNS. Admin bearer authentication remains a separate boundary. | Native request and CONNECT checks recognize local names and numeric aliases with one trailing DNS root dot before credential admission. Before connecting, the sole egress path checks each selected socket against the protected local addresses and actual bound listener. The same port at a remote address or 127.0.0.2 remains allowed. Invalid numeric extra-port configuration rejects the candidate and preserves the previous live snapshot. The [shield tests](../proxy/tests/admin_shield.rs) compare the changed source hook and its remaining DNS alias gap; the [operator transport tests](../proxy/tests/admin_transport.rs) exercise the integrated boundary. The proxy checks the immediate configured parent socket; origin resolution beyond that parent remains the parent's responsibility. |
...
| D57 | A source flow-record tag failure can leave the inserted flow pending on its SQLite connection. A later successful operation commits that failed record. | Native recording rolls the row and provenance tags back together, while retaining separate best-effort body search indexing. The [storage comparison](../proxy/tests/flow_store.rs) preserves the source witness and checks that the failed record stays absent after another commit and reopen. The live [`live_evidence_sink_failure_keeps_transport_and_capture_distinct_from_recovery`](../proxy/src/flow_runtime_tests.rs) control replaces the response-phase event sink with writable `/dev/full`: the origin still returns 200, the response marks the evidence failure, the captured row survives reopen, and a later healthy write has no stale failure marker. That live sink control covers one response-phase event sink. Native Agent API and Plumb approval submissions separately wait for the canonical audit append before reporting a pending request; [`approval_audit_delivery`](../proxy/tests/approval_audit_delivery.rs) checks an unwritable destination, empty operator views and same-process recovery. The retained Python AgentAPI now waits for append/close before claiming operator review for service access, contract binding, desktop presentation or a Plumb chat request; failed writes return 500, while ordinary audit events stay asynchronous. These waits run off the event loop so other AgentAPI calls remain available. The [real-proxy control](../tests/proxy_migration/test_python_agent_api_approval_audit.py) checks failure, both operator views and same-process recovery. Native and retained Python service-gateway risky-route approval prompts also wait for the audit append before returning 428. With an unwritable destination, native returns its existing 502 proxy error and retained Python returns its existing 503 failure response; neither failed request appears in the operator approval views. After the destination is restored, a new prompt becomes visible in the same process. The [shared real-proxy control](../tests/proxy_migration/test_gateway_risk_approval.py) checks failure, recovery and an ordinary allowed route on both backends. Native and retained Python network-guard egress and credential-guard prompts also wait for the canonical audit append before reporting a pending approval. If the append fails, native returns 502 and retained Python returns 503 without forwarding to the origin or adding a new request to either operator approval view. A new prompt becomes visible after storage recovers in the same process. The [shared guard control](../tests/proxy_migration/test_guard_approval_audit.py) checks both backends, both operator views, credential isolation, and unrelated allow and deny decisions. These confirmations do not promise fsync or crash durability. |
| D58 | Direct source flow reads fall back to legacy agent_id when a schema-v2 row has no authoritative evidence_owner. An explicitly quarantined owner-null row is therefore readable by that legacy agent although scoped search excludes it. | Native direct reads require exact evidence_owner, matching collection scope. Foreign, unresolved, quarantined and missing records share the existing 404. [API tests](../proxy/tests/agent_api_flows.rs) prove denial before loading or decompressing a body; the `evidence_owner_is_the_single_scope_for_search_detail_body_and_tags` witness extends that check across Alice/Bob collection, detail, body and tag paths, while [trace tests](../proxy/tests/agent_api_trace.rs) cover foreign and owner-null trace records. Existing version-1 migration still assigns owners; reads do not reattribute quarantined version-2 evidence. |
...
| D63 | Earlier native forced shutdown aborted outer connection tasks and dropped nested JoinSets or driver handles without joining their descendants. WebSocket close events and driver cleanup could then follow client removal or audit shutdown. | One accepted-connection task owner now retains explicit HTTP drivers, CONNECT/WS work, Hyper transport-executor jobs and actual WS scanner jobs through cancellation. Tasks registered after cancellation are dropped before their work runs. The client guard ends after that owner drains. The desktop presenter now keeps child-process ownership separate from its protocol I/O lock, so shutdown can kill and reap a helper even when a canceled presentation request is blocked waiting for a response; `desktop_present::tests::shutdown_reclaims_helper_after_canceled_blocked_request` proves that bounded case. Finite owner and owned H1/WS tests establish this transport scope; standalone API workers, anonymous spill-file jobs and ordinary process Drop remain outside the guarantee. The real-NATS [`coord_wait_shutdown`](../proxy/tests/coord_wait_shutdown.rs) witness now also shuts down a pending coordination subscription, independently observes its subscription removal, reopens the same state and socket paths with a distinct instance identity, repeats the wait, and verifies that neither shutdown appends a late audit or event line. The candidate `admin_api::services::tests::admitted_service_mutation_distinguishes_queue_full_submission_failure` additionally drives a real service mutation while its same-process audit FIFO is held/full: policy persistence, FIFO rows and writer reservations are observed independently, and a synchronous audit submission failure is not reported as durable service evidence. [Candidate slice; graceful process shutdown and the complete producer matrix remain open.] |
...
| D68 | At `31df5ceb`, native HTTP restores client `TE: gzip` after removing `Connection: TE`, so an origin receives a nominated hop header. It also strips an upstream `Transfer-Encoding: gzip, chunked` declaration while forwarding the gzip coded bytes. The Python comparator removes the nominated request field and relays the response coding. | The native candidate removes the request `TE` restoration while keeping the separate request `Trailer` declaration. It retains the upstream non-chunked transfer coding when Hyper regenerates HTTP/1.1 chunk framing. If the client protocol cannot carry that coding, the proxy returns 502 before releasing the response body. The shared [raw-wire fixture](../tests/proxy_migration/test_te_gzip_handoff.py) covers the gzip cases, a plain-chunked control, and a second response on the same client connection. The HTTP/1 handoff was accepted at PR #743. The shared TLS/HTTP/2 case observes h2 on the client and http/1.1 at the origin: Python completes valid gzip transfer coding with gzip bytes but no coding declaration, while native returns its documented 502. Both complete plain chunked and the next stream. When the first plain-chunked size line is invalid, Python returns 502. Native resets only that HTTP/2 stream with INTERNAL_ERROR before sending response headers or data. The next stream still completes. This case does not establish transfer-coding conversion. |
...
after network admission. For CONNECT, an authored logical destination or SNI
entry can select opaque transport through a configured parent. The physical
connection still uses that parent. The builtin `api.asterfold.ai:7000` entry
uses the same matcher. Other recognized opaque traffic also retains arbitrary
...
addresses. A configured exact logical destination or `SNI:port` alias can be
selected on a direct or parent CONNECT route. The proxy sends the original
CONNECT authority to the parent, and the lifecycle event records the matched
SNI when an alias selected the connection. Direct IPv4 range matches discovered
from the connected peer retain the same lifecycle owner. A parent address or
CIDR never becomes a matching input for an unrelated logical destination. The
native [inner-Host control](../proxy/tests/transport.rs) keeps a
...
witnesses. The live
[`native_pattern_scanner_decodes_gzip_response_on_real_h2`](../proxy/tests/credential_http.rs)
...
Opaque delegated-search mid-search cancellation remains an explicit coverage
limit; streamed request bodies above the 10 MiB threshold and configured SSE
responses forward with an unavailable body and no scan claim (188a5291).
Python 3.12 accepts
...
broader regex grammar and uncovered Unicode properties, with unsupported text
failing closed, remain explicit compatibility gaps; streamed bodies above the
10 MiB threshold and configured SSE responses forward with an unavailable body
and no scan claim (188a5291), and full production-chain integration remains a
#621/#636/#637/#640 dependency. Accepted
...
For admitted CONNECT requests, the native proxy emits canonical
...
existing passthrough configuration, including through a configured parent. A
direct configured IPv4 range can also match the resolved peer after TCP
succeeds. The [connection component](../proxy/src/ignored_host_logger.rs) uses
the shared audit writer. The events retain the source host, port, transport and
trusted listener agent/client facts. They contain no request ID, explicit
attribution, byte count or claim about inspected application content.

Logical matching happens before direct DNS or parent connection setup and is
retained for that physical connection. A resolved-peer-only match is selected
only after a direct TCP connection succeeds; its event keeps the logical
destination while the matcher uses the physical IPv4 peer. A direct logical
match starts after TCP succeeds. A parent logical match starts only after the
parent accepts CONNECT. If the parent rejects CONNECT before destination
transport starts, the pending owner records one error without a start/end pair.
...
The current runtime scope excludes ordinary HTTP connections and inner-Host
aliases. The native [SNI alias transport controls](../proxy/tests/transport.rs)
cover one exact configured `SNI:port` alias through direct and parent routes,
with origin-certificate and initial-record evidence plus an intercepted
neighboring SNI. Exact parent-address and parent-CIDR entries leave an
unrelated logical destination intercepted. The remaining lifecycle matrix and
full D29 acceptance remain unresolved. Earlier native reserved/admin
containment can also omit source connection observations; enforcement order
remains unchanged.
...
passthrough entry selects opaque transport while still sending the CONNECT and
client payload to the configured parent. An independently listening origin
accepts zero connections. The parent records the logical authority and its
loopback peer. The single `proxy.egress` event keeps the logical host and port
with `route: "parent"`, and the canonical start/end pair uses the logical
destination. Companion controls cover a verified TLS parent, a configured SNI
alias through a parent, a same-host different-port neighbor, and exact
parent-address/CIDR entries for an unrelated destination. A parent refusal
records one error without a direct fallback or start/end pair. These are focused
native witnesses. They do not establish reserved-name controls, same-request
retry, alternate-parent behavior or full D29 acceptance.
...
subscription is independently observed and removed, and audit/event files
remain unchanged after each owner drain. This is one coordination worker
slice, not the complete #626/#628/#629 standalone-worker matrix. Blocking
