...
| 16. `addons/credential_guard.py` | Detect HTTP request-header credentials, evaluate destination-first permissions and budgets, and return configured block/warn outcomes. | Native H1 request-head and H2-inside-owned-TLS request-head integration run after network/circuit admission and before observation/body/dial; policy events use keyed fingerprints and identity conflicts fail closed. Bodies and WebSockets belong to the pattern scanner. | Native header detection/selection and policy evaluation are active for `policy_file`; malformed header bytes cross a reversible private source-text adapter and controlled H1/H2 wires prove matching warn/block plus nonmatching raw forwarding. Body/query scanning remains outside this adapter. D33 named-Unicode grammar, uncovered Unicode properties/casefold behavior, nesting depth, large-message limits, HTTP text decoding and full production-chain integration remain open. | `test_credential_guard.py`, `test_credential_catalog.py`, `test_policy_budget_contract.py`, `credential_http.rs`, `http::traffic::tests::ordinary_h2_inside_owned_tls_logs_inner_exchange_only` |
...
| D33 | fancy-regex does not reproduce all accepted Python regular expressions. Repairs cover scoped ASCII flags, octal escapes, Turkish-I literals and Python backreference comparisons. Named-Unicode escapes, uncovered grammar and parser-depth differences remain explicit compatibility failures. Its original private one-million-entry stack limit also makes `(a\|aa)*\1$` fail on 1,000,100 `a` bytes where Python matches. The scanner's existing error rule then drops even a log-mode message. | A [pinned source patch](../proxy/vendor/fancy-regex/SAFEYOLO.md) removes that private cutoff with fallible VM growth and releases per-search buffers. Python/native log-mode matches now agree at 1,000,100, 4 MiB and 8 MiB. Generated Python 3.12 / Unicode 15 ranges pin `\w` and `\d`; U+13460 and U+1E4F0 witnesses cover a newer-table category mismatch and a Unicode-15 decimal digit. Finite nesting, named-Unicode rejection, backreferences and scanner error/warn/block semantics have focused witnesses. The native WS reader and scanner test actual fragmentation, control frames and per-message deflate, and relay cancellation prevents message publication. Opaque delegated-search cancellation, HTTP text decoding, broader regex grammar and full production-chain integration remain open. No message cap was added to hide those differences. |
...
Generated ranges pin Python 3.12 / Unicode 15 `\w` and `\d` membership, while
named Unicode escapes, uncovered properties and deeper grammar remain explicit
compatibility failures. Finite nesting, scanner error/warn/block behavior,
compressed fragmented messages with control frames and cancellation before
evidence publication have focused native witnesses. Opaque delegated-search
cancellation, the HTTP text adapter and full production-chain integration
remain unresolved. Full details and pinned data provenance are in the [engine
patch](../proxy/vendor/fancy-regex/SAFEYOLO.md).
...
named-Unicode grammar, uncovered Unicode properties, HTTP text decoding and
full production-chain integration remain explicit compatibility gaps. Accepted
