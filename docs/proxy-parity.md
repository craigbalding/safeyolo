...
| 16. `addons/credential_guard.py` | Detect HTTP request-header credentials, evaluate destination-first permissions and budgets, and return configured block/warn outcomes. | Native H1 request-head integration runs after network/circuit admission and before observation/body/dial; policy events use keyed fingerprints and identity conflicts fail closed. Bodies and WebSockets belong to the pattern scanner. | Native header detection/selection and policy evaluation are active for `policy_file`; remove addon and flow-to-policy adapters when replaced. Raw invalid-byte header parity (D48) and body/query scanning remain explicit gaps. | `test_credential_guard.py`, `test_credential_catalog.py`, `test_policy_budget_contract.py`, `credential_http.rs` |
...
addon-enable queries. Native request-head activation is covered by the focused
wire candidate but independent acceptance remains pending. Header values that
contain invalid UTF-8 are rejected by the strict text adapter; source D48
surrogateescape behavior remains an explicit gap, as do existing regex
compatibility gaps. Accepted
