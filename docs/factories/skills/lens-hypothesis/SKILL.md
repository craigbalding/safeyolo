...
Start from a working request or sequence through the same entry point users or agents reach. Before interpreting generated cases, prove the fixture exercises the feature with a **feature-dependent control**: for example, a gateway injects its vault credential into a permitted request, or a standalone provider-shaped header produces the expected classified credential decision. A generic success or denial is insufficient. If the control fails, fix the fixture or call the probe inconclusive. State the requirement as an observable property before choosing payloads. Identify the attacker-controlled fields, the trust boundary, and an oracle independent of the candidate implementation. The HTTP status alone is usually insufficient: inspect the caller-visible response **and** the effect that matters, such as the upstream sinkhole request, authorization decision, stored state, or connection state. For Python/Rust parity, replay the same generated case against both, but check the promised decision and effect rather than requiring identical response fields; two implementations can agree on a wrong result.

Build a small, valid grammar around that baseline. Spend generated cases on dimensions that could change the decision, especially exact boundaries; avoid random fields or sequence bits that cannot affect the property:
...
Read [proxy review examples](references/proxy-review-examples.md) for practical query/header, credential, and connection-sequence patterns. The [encoded gateway alias](references/gateway-encoded-alias.md) and [reused connection](references/reused-connection.md) references show tested fixture controls, generators, and effect checks when those boundaries are relevant. Adapt them to the current feature; they are leads, not a mandatory catalog.
