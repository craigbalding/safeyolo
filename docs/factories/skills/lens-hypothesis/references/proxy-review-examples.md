...
Generate synthetic token-like text with variable prefix, length, delimiter, casing, surrounding prose, and placement in headers, query, or body. Keep separate valid-positive and valid-negative strategies so a run is not dominated by malformed noise. Check the decision and the upstream/log effect. If testing an authorization rule, substitute the principal or scope independently of the object ID, then explicitly test their combination. A 428, 403, or 200 response by itself does not prove the property. Do not put real service credentials in generated cases or failure reports.
