...
Before merging after Lens READY and Forge DONE, Relay compares the assigned
outcome, any review instruction in this brief that plainly applies, and Lens's
reported result. Hold the merge for a specific gap: no independent observed
result for the claimed boundary; a required criterion treated as proved despite
a stated limitation; or an applicable review method omitted or replaced without
a concrete reason. For structured input or meaningful request/state sequences,
a fixed case list does not satisfy the Hypothesis instruction. Lens's disposition
must briefly name the generated input dimensions and observed real effect, or
give a concrete candidate-specific reason generation cannot test the claim.
Ask Lens one bounded TASK about the exact commit
and use its DONE before deciding. Do not make this a routine second review or
request extra artifacts or broad CI. If the answer invalidates READY, keep the
merge held, have Lens correct any premature acceptance record, and use the
normal Forge correction and Lens review path.
