...
| M7-05 | `cli/src/safeyolo/proxy_modes/__init__.py` | Python UDS mode package boundary; runtime registration is owned by `proxy_modes/unix_listener.py::ensure_registered()` | Native ingress replaces the `unix_listener.py` registration owner and no Python mode import remains on the selected Rust path | `tests/test_unix_listener.py` | retained |
...
| M8-33 | `cli/src/safeyolo/websocket_body_filter.py` | Python WebSocket body-filter and display rendering patch | Equivalent native WebSocket body filtering, display, and unsupported-outcome behavior are demonstrated before removal | `cli/tests/test_websocket_body_filter.py`<br>`tests/proxy_migration` | retained |
