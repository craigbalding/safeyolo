...
The currently executable owned transition is the circuit row. With
`SAFEYOLO_PYTHON_SOURCE` set to the comparator checkout and
`SAFEYOLO_RUST_PROXY` set to the candidate binary, the migration test writes an
open circuit in the Python process, reads and recovers it in Rust, then reopens
the closed state in Python. Its manifest contains SHA-256 state hashes and
effective origin-contact counts; it contains no secret material. The remaining
rows require their final integrated writer owners before they can be promoted
to the same process sequence.
