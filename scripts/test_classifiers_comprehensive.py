#!/usr/bin/env python3
"""
Comprehensive test suite comparing DeBERTa vs phi3.5 for prompt injection.

Adapted from tourism/gateway/tests/test_classifier_comprehensive.py
"""
...
class DebertaClassifier:
    ...
    def classify(self, text: str, threshold: float = 0.5) -> tuple[Classification, float, float]:
...
def main():
    ...
    print("Loading phi3.5 classifier...")
    phi35 = Phi35Classifier()

    # Run tests
    deberta_results = run_tests(deberta, "DeBERTa ONNX", verbose=True)
    phi35_results = run_tests(phi35, "phi3.5 (Ollama)", verbose=True)
    ...
    print_summary(deberta_results, "DeBERTa")
    print_summary(phi35_results, "phi3.5")

    print("\n" + "=" * 70)
    print("RECOMMENDATION")
    print("=" * 70)

    d_acc = 100 * deberta_results["correct"] / deberta_results["total"]
    p_acc = 100 * phi35_results["correct"] / phi35_results["total"]
    d_fp = deberta_results["false_positives"]
    p_fp = phi35_results["false_positives"]
    d_fn = deberta_results["false_negatives"]
    p_fn = phi35_results["false_negatives"]
    d_lat = deberta_results["total_latency_ms"] / deberta_results["total"]
    p_lat = phi35_results["total_latency_ms"] / phi35_results["total"]

    print(f"\n| Model   | Accuracy | FP  | FN  | Latency |")
    print(f"|---------|----------|-----|-----|---------|")
    print(f"| DeBERTa | {d_acc:6.1f}%  | {d_fp:3} | {d_fn:3} | {d_lat:5.0f}ms |")
    print(f"| phi3.5  | {p_acc:6.1f}%  | {p_fp:3} | {p_fn:3} | {p_lat:5.0f}ms |")

    if p_fp < d_fp and p_fn <= d_fn:
        print("\nphi3.5 has fewer false positives with equal/better false negative rate.")
        print("Consider using phi3.5 as primary (async, warn-only) despite higher latency.")
    elif d_fp < p_fp:
        print("\nDeBERTa has fewer false positives. Consider raising threshold.")
    else:
        print("\nBoth have trade-offs. Consider layered approach.")
