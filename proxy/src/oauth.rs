...
fn decode_json_text(body: &[u8]) -> Result<Zeroizing<String>> {
    ...
    match encoding {
        Encoding::Utf8 => {
            text.push_str(std::str::from_utf8(body).map_err(|_| RefreshError::JsonEncoding)?)
        }
        Encoding::Utf16(little) => {
            if !body.len().is_multiple_of(2) {
                return Err(RefreshError::JsonEncoding);
            }
            ...
            for character in char::decode_utf16(words) {
                text.push(character.map_err(|_| RefreshError::JsonEncoding)?);
            }
        }
        Encoding::Utf32(little) => {
            ...
            for bytes in body.chunks_exact(4) {
                ...
                text.push(char::from_u32(point).ok_or(RefreshError::JsonEncoding)?);
            }
        }
    }
    Ok(text)
