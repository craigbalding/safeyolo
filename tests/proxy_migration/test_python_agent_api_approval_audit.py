...
def _send(proxy, agent, path, body=None):
    ...
    with (proxy.event_log.parent / "agent-api-approval-wire.jsonl").open("a") as output:
        output.write(
            json.dumps(
                {
                    "agent": agent,
...
def _operator_rows(api, log_path, stage):
    ...
    with (log_path.parent / "operator-views.jsonl").open("a") as output:
        output.write(json.dumps({"stage": stage, "admin": admin_rows, "watch": watch_rows}) + "\n")
...
""")
    ...
    with launch_proxy(
        ...
        rows = _operator_rows(api, audit, "healthy")
        ...
        try:
            ...
            rows = _operator_rows(api, audit, "failed")
            assert {(row["event"], row["agent"]) for row in rows} == {(event, "bob") for event in APPROVAL_EVENTS}
        finally:
            audit.chmod(0o644)

        dave = _submit_all(proxy, "dave", 202)
        rows = _operator_rows(api, audit, "restored")
