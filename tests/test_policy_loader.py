...
class TestSighupHandling:
    """Tests for SIGHUP signal handling."""

    def test_sighup_handler_is_async_signal_safe(self):
        """Test SIGHUP handler only sets event (no I/O, no locks)."""
        from policy_loader import PolicyLoader

        loader = PolicyLoader()

        # Handler should only set the event
        loader._handle_sighup(None, None)

        # Event should be set
        assert loader._sighup_event.is_set()

    def test_sighup_worker_thread_starts(self):
        """Test SIGHUP worker thread starts on initialization."""
        from policy_loader import PolicyLoader

        loader = PolicyLoader()

        # Thread should exist and be alive
        assert loader._sighup_thread is not None
        assert loader._sighup_thread.is_alive()
        assert loader._sighup_thread.daemon is True
        assert loader._sighup_thread.name == "sighup-reload"

    def test_sighup_triggers_reload(self):
        """Test setting SIGHUP event triggers policy reload."""
        from policy_loader import PolicyLoader
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            reload_called = []
            loader = PolicyLoader(baseline_path=path, on_reload=lambda: reload_called.append(1))

            # Initial load triggers on_reload once
            ...
            path.write_text(
                "permissions:\n"
                "  - action: network:request\n"
                "    resource: updated.com\n"
                "    effect: allow"
            )

            # Simulate SIGHUP (set event directly - same as signal handler)
            loader._sighup_event.set()

            # Wait for worker to process
            time.sleep(0.2)

            # Reload should have been called once more
            assert len(reload_called) == initial_count + 1
            assert loader.baseline.permissions[0].resource == "updated.com"
