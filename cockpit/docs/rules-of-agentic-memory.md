...
With the term `agentic memory file` we refer to any `agents.memory.md` named file of a given module within the relevant `modules of this scope`:
    ...
    - Similarly named files in other scopes are not be modified.
...
Duh, critical learnings means substantial or very important for subsequent agentic sessions information or methods, so it's not about taking notes and the blahblah llm-based tools tend to generate along valuable information. Knowledge we want to persist is like:
    * Undocumented details that we learnt by testing and tial/errors.
    * Details that we construed from processing mulitple sources (expensive queries and thinking).
...
* We do not bloat persistant memory with stuff (derivative knowledge) that we anytime can reconstruct from primary pieces of knowledge we had to record.
...
2. The main rule regarding recording and updating marked context as persistant memory is that modification of the `agents.memory.md` files can only be done when the user/human instructs to doing so.
